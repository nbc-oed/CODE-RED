...
export class DestinationRiskController {
  ...
  async checkDestinationRisk(@Query('destination') destination: string) {
    const destinationRisk =
      await this.destinationRiskService.checkDestinationRisk(destination);
  ...
  async getUserCoordinate(@Query('x') x: string, @Query('y') y: string) {
    ...
    const myLocation = await this.destinationRiskService.getUserCoordinate(
      longitude,
      latitude,
    );
    return myLocation;
  ...
  async getUserRegionCoordinate(@Query('x') x: string, @Query('y') y: string) {
    const longitude = parseFloat(x);
    const latitude = parseFloat(y);
    const myRegion = await this.destinationRiskService.getUserRegionCoordinate(
      longitude,
      latitude,
    );
    return myRegion;
