...
export class DestinationRiskService {
  ...
  async findRisk(destination: string) {
    try {
      const realTimeDataJsonVer =
        await this.seoulCityDataXmlToJson(destination);
      const areaName =
        realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA']['AREA_NM'][
          '_text'
        ];
  ...
  async detailCheck(destination: string) {
    try {
      ...
      const destinationRainOrSnowNews =
        realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA']['WEATHER_STTS'][
          'WEATHER_STTS'
        ];
  ...
  async destinationRiskDetailedInquiry(destination: string) {
    ...
    try {
      ...
      const closeToDestination = await this.destinationRepository
        ...
        .getRawOne();
      if (closeToDestination) {
        const realTimeDestinationRiskDetailInquiry = await this.detailCheck(
          closeToDestination.area_name,
        );
        return realTimeDestinationRiskDetailInquiry;
      } else {
  ...
  async savedDestination() {
    ...
    for (const destination of destinationArr) {
      try {
        ...
        const areaCode =
          realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA']['AREA_CD'][
            '_text'
          ];

        // 지하철, 정류장 좌표 둘 다 없고 도로 좌표가 하나라면
        if (
          (realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA']['SUB_STTS'] ===
            undefined ||
            Object.keys(
              realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA']['SUB_STTS'],
            ).length === 0) &&
          (realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA'][
            'BUS_STN_STTS'
          ] === undefined ||
            Object.keys(
              realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA'][
          ...
          const xyCoordinates =
            realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA'][
              'ROAD_TRAFFIC_STTS'
            ]['ROAD_TRAFFIC_STTS']['START_ND_XY']['_text'];
        ...
        // 지하철, 정류장 좌표 둘 다 없고 도로 좌표가 여러개라면
          ...
          const xyCoordinates =
            realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA'][
              'ROAD_TRAFFIC_STTS'
            ]['ROAD_TRAFFIC_STTS'][0]['START_ND_XY']['_text'];
          const coordinates = xyCoordinates.split('_');
          const savedDestination = {
            areaName,
            areaCode,
        ...
        // 지하철 좌표가 없고 정류장이 하나라면
          ...
          realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA']['BUS_STN_STTS'][
            'BUS_STN_STTS'
          ][0] === undefined
        ) {
          const areaLongitude =
            realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA'][
              'BUS_STN_STTS'
            ]['BUS_STN_STTS']['BUS_STN_X']['_text'];
          const areaLatitude =
            realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA'][
              'BUS_STN_STTS'
            ]['BUS_STN_STTS']['BUS_STN_Y']['_text'];
          const savedDestination = {
            areaName,
            areaCode,
            areaLongitude,
          ...
          const areaLongitude =
            realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA'][
              'BUS_STN_STTS'
            ]['BUS_STN_STTS'][0]['BUS_STN_X']['_text'];
          const areaLatitude =
            realTimeDataJsonVer['SeoulRtd.citydata']['CITYDATA'][
              'BUS_STN_STTS'
            ]['BUS_STN_STTS'][0]['BUS_STN_Y']['_text'];
          const savedDestination = {
            areaName,
            areaCode,
            areaLongitude,
  ...
  async updatedDestination(destination: string) {
    try {
      ...
      if (
        ...
        if (findDestination) {
          await this.destinationRepository.update(
            { area_code: updatedDestination.areaCode },
            {
              area_name: areaName,
              area_code: areaCode,
